...
backup_path = js_path.with_suffix(js_path.suffix + ".bak_before_avatar_normal_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
...
new_js = r'''document.addEventListener("DOMContentLoaded", function () {
    ...
    function getCropDiameter() {
        if (!cropper) return 280;

        const containerData = cropper.getContainerData();
        const maxWidth = Math.max(180, containerData.width - 64);
        const maxHeight = Math.max(180, containerData.height - 64);

        return Math.round(Math.min(320, maxWidth, maxHeight));
    }
...
print("OK: Avatar editor je zamijenjen normalnom verzijom.")
