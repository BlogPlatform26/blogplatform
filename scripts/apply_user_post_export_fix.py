...
URL_LINE = '    path("postavke/izvoz-postova/", export_my_posts, name="export_my_posts"),\n'
...
def update_urls():
    ...
    if IMPORT_LINE.strip() not in text:
        # Dodaj import na početak, ali poslije eventualnog coding komentara/docstringa ne diramo jer nije bitno za ovaj file.
        text = IMPORT_LINE + text

    if 'name="export_my_posts"' not in text and "name='export_my_posts'" not in text:
        marker = "urlpatterns = ["
        if marker not in text:
            raise RuntimeError("Ne mogu pronaći 'urlpatterns = [' u blog/urls.py")
        text = text.replace(marker, marker + "\n" + URL_LINE, 1)

    if text != original:
        backup(URLS_PATH, ".bak_before_user_post_export")
        URLS_PATH.write_text(text, encoding="utf-8")
        print("Ažurirano: blog/urls.py")
    else:
        print("blog/urls.py već ima export rutu.")
...
def update_settings_tab():
    ...
    if text != original:
        backup(SETTINGS_TAB_PATH, ".bak_before_user_post_export")
