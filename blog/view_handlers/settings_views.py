    ...
    if request.method == 'POST':
        ...
        if request.GET.get('tab') == 'postavke':
            ...
            if settings_tab == 'opcenito':
                ...
                if selected_banner_size in {'xsmall', 'small', 'medium', 'large', 'full_width'} and profile.blog_banner_size != selected_banner_size:
...
def deactivate_account(request):
    if request.method == "POST":
        ...
        if confirm_text != "deaktiviraj":
            messages.error(request, 'Za potvrdu moraš upisati "deaktiviraj".')
            return redirect("deactivate_account")

        if request.user.has_usable_password() and not request.user.check_password(password):
            messages.error(request, "Lozinka nije ispravna.")
            return redirect("deactivate_account")
        ...
        messages.success(
            request,
            "Tvoj račun je deaktiviran. Ako ga želiš ponovno aktivirati, javi se podršci."
        )

        return redirect("home")

    return render(request, "blog/account_deactivate_confirm.html")
